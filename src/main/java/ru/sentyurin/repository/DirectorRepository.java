...
public class DirectorRepository implements Repository<Director, Integer> {
	...
	private static final String GET_ALL_MOVIES_BY_DIRECTOR_ID_SQL = """
			select m.id as id,
	...
	public DirectorRepository() {
		...
		movieResultSetMapper = new MovieResultSetMapper();
	}
	...
	public Optional<Director> findById(Integer id) {
		try (Connection connection = connectionManager.getConnection();
			...
			director.setMovies(findMoviesByDirectorId(id));
	...
	private boolean deleteByIdSupporting(Integer id) throws SQLException {
		...
		try {
			...
			return deleteDirectorAndTheirMovies(id, connection);
		} catch (Exception e) {
			if (connection != null)
				connection.rollback();
			throw e;
	...
	private boolean deleteDirectorAndTheirMovies(int id, Connection connection)
			throws SQLException {
		try (PreparedStatement directorDeletion = connection.prepareStatement(DELETE_BY_ID_SQL);
				PreparedStatement movieDeletion = connection
						.prepareStatement(DELETE_MOVIES_BY_DIRECTOR_ID_SQL)) {
			connection.setAutoCommit(false);
			directorDeletion.setInt(1, id);
			movieDeletion.setInt(1, id);
			movieDeletion.executeUpdate();
			int linesDeleted = directorDeletion.executeUpdate();
			connection.commit();
	...
	private List<Movie> findMoviesByDirectorId(int id) {
		try (Connection connection = connectionManager.getConnection();
				PreparedStatement statement = connection
						.prepareStatement(GET_ALL_MOVIES_BY_DIRECTOR_ID_SQL)) {
			statement.setInt(1, id);
			return movieResultSetMapper.map(statement.executeQuery());
		} catch (SQLException e) {
			throw new DataBaseException(e.getMessage());
		}
	}
